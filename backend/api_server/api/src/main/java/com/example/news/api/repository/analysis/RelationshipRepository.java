...
import com.example.news.api.dto.response.analysis.EventTrackerResponse;
import com.example.news.api.dto.response.analysis.GraphResponse;
import com.example.news.api.dto.response.analysis.PowerCoupleResponse;
...
import com.example.news.api.util.mapper.AggregationMapper;
...
public class RelationshipRepository {
    ...
    private final AggregationMapper aggMapping;
    ...
    public List<PowerCoupleResponse> getPowerCoupleWithRelativeInterval (String intervalUnit, int amount){
        ...
        return dsl.select(
            ...
            .limit(50)
            .fetchInto(PowerCoupleResponse.class);
    ...
    public List<EventTrackerResponse> getEventTrackerWithRelativeInterval (String intervalUnit, int amount){
        ...
        return dsl.select(
            ...
            .orderBy(DSL.count().desc())
            .limit(50)
            .fetchInto(EventTrackerResponse.class);
    }

    public List<DetailedEntity> findEntitiesByNewsLink(String newsLink) {
        return dsl.select(
                ...
                .fetchInto(DetailedEntity.class);

    }
    public void syncNewsToNeo4j() throws SQLException {
        List<DetailedNewsResponse> newsList = new ArrayList<>();
        ...
        try (Session session = this.neo4jDriver.session()) {
            for(DetailedNewsResponse news : newsList){
    ...
    public void syncNewsEntityToNeo4j() throws SQLException {
        List<Neo4jEntity> newsEntitiesList = new ArrayList<>();
        ...
        try (Session session = this.neo4jDriver.session()) { 
            for (Neo4jEntity newsEntity : newsEntitiesList){
                Map<String, Object> params = new HashMap<>();
    ...
    public GraphResponse getDiscoveryData(String intervalUnit, int amount) {
        ...
        try (Session session = this.neo4jDriver.session()) {
            session.executeRead(tx -> {
                ...
                while (result.hasNext()) {
                    ...
                    // Build unique nodes
                    // Update Source Node
                    nodeMap.putIfAbsent(sName, new GraphNode(sName, sName, sGroup, 0, 0));
                    GraphNode sNode = nodeMap.get(sName);
                    sNode.setSize(sNode.getSize() + w); // Pass the new calculated size
                    sNode.setSentiment((sNode.getSentiment() + s) / 2);

                    // 2. Update Target Node
                    nodeMap.putIfAbsent(tName, new GraphNode(tName, tName, tGroup, 0, 0));
                    GraphNode tNode = nodeMap.get(tName);
                    ...
                    response.getLinks().add(new GraphLink(sName, tName, w, s));
                }
                return null;
            });
        }
